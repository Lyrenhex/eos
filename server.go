...
type Year struct { // Year structure to create a copy of a year's Moods structure
  Year  int
  Day   [7]Mood
  Month [12]Mood
}
  ...
  http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request){
    ...
    for alive {
      ...
      case "mood":
        ...
        users[data.UserID].Moods.Day[payload.Day].Num += 1
        users[data.UserID].Moods.Month[payload.Month].Mood += payload.Mood
        users[data.UserID].Moods.Month[payload.Month].Num += 1
